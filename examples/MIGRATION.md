...
Going forward, `strands-robots-sim` is the home for the **heavy,
NVIDIA-GPU-only** Isaac Sim simulation backend that plugs into
`strands-robots` through its `SimEngine` ABC. See the umbrella issue
