...
def _default_checkpoint_dir() -> str:
    ...
    path" guard). Those two defaults are mutually inconsistent, so the OOTB
    ``--policy groot`` lifecycle aborts at ``start_container`` (strands-labs/robots-sim#125).
...
def _explain_lifecycle_failure(result: dict, checkpoint_dir: str, container: str) -> str:
    ...
    * the ``/home`` "protected host path" mount guard (strands-labs/robots-sim#125),
...
def main() -> None:
    ...
    if args.policy == "groot" and args.auto_server:
        ...
        # mount guard (strands-labs/robots-sim#125). When the user passes
        # `--checkpoint-dir` explicitly we honor it verbatim.
        ...
        # check fires once the model is resident (the old 10 GiB gate never
        # tripped — the model footprint is below it — so --auto-server always
        # timed out at 180 s).
